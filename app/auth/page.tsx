...
const Page = () => {
  ...
  useEffect(() => {
    ...
    if (token !== null && token.length > 0) {
      router.push("/dashboard");797769294
    }
  }, [router]);
  ...
  const onNext = async () => {
    ...
    try {
      const url = `${getUmsBaseUrl()}/auth/check-registration?MobileNumber=${mobileNumber.current}`;
  ...
  const gridConfig: GridConfig[] = [
    {
      ...
      view: (
        <CardView className="flex flex-col justify-center items-center">
          ...
          />
          {error.current && <p className="text-red-500 mt-2">{error.current}</p>}
          <div className="mt-4">
            <Button
              variant="contained"
              style={{
